...
# 실호출로 수 분씩 걸려 기본 pytest를 느리게(hang처럼) 만든다. 실제 임베딩 모델(real_embedding)도
# 모델 로드와 HF 허브 접근이 걸려 같은 취급이다. 조직 원칙(네트워크·실서비스 없이 빠르게 통과)을
# 지키기 위해, 이름에 real_ollama/real_claude/real_embedding이 든 스모크는 기본 실행에선 건너뛴다.
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        if any(tag in item.name for tag in ("real_ollama", "real_claude", "real_embedding")):
