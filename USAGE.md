...
- **로컬 추출이 너무 느리면** `.env`에 `EXTRACTION_ENTITY_TYPE_SPLIT=false`. 로컬 백엔드의 분해 추출은 기본적으로 엔티티를 타입군 4개로 나눠 물어 **청크당 5콜**(엔티티 4 + 관계 1)이 나가는데, 이걸 끄면 통합 질문 1콜로 줄어 **청크당 2콜**이 됩니다(콜 수 60%↓). 대신 한 번에 넓게 묻는 만큼 누락이 늘 수 있어 기본값은 분할(켬)입니다 — 이 분할이 로컬 모델의 과부하 누락을 되돌린 장치입니다.
- **긴 문서(소설 등)를 더 빨리 처리하려면** `--profile fast`(청크당 2콜) 또는 `--profile auto`(문서 길이로 자동 선택). 기본은 `quality`(청크당 5콜, 무손실)이고, `--profile`이 새 단일 제어입니다 — 위 `EXTRACTION_ENTITY_TYPE_SPLIT` 토글은 `--profile` 미지정 시에만 쓰이는 폴백입니다.
