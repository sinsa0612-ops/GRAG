...
## 10. retrieval 투자 1차 — 답변 합성 충실성 재구성 (구현 완료 / 전체 eval 리셋 후)

### 기능 체감
- **환각 억제:** 답변이 이제 **본문 조각을 1차 근거, 그래프를 보조 힌트**로 쓴다. 그래프에만 있고 본문에 근거 없는 내용(과잉 추출이 만든 엉뚱한 연결)을 사실로 단정하지 않고, 본문과 충돌하면 본문을 따른다. → 추출이 풍부한 조건에서 관찰되던 그래프발 환각(예: 스크루지 성격을 '난로'에 오귀속) 위험을 줄인다.
- 벡터 근거 조각을 `[본문 N]`으로 번호 매기고 완전 중복을 제거해 근거 경계가 분명해진다.

### 변경 파일
- `query.py`: `_ANSWER_PROMPT` 재구성(본문 1차·그래프 보조·충돌 시 본문 우선·미검증 그래프 주장 단정 금지, 본문 블록을 앞으로), `_build_vector_context`(dedup+번호 라벨) 신규, `answer_question` 배선. **스키마·DAL·인터페이스 무변경**(query.py만).
- `tests/test_query.py` +3(dedup·라벨 / 빈 입력 / 본문 우선 프레이밍).

### 검증 (✅ 완료 — 리셋 후 전체 eval)
- `pytest` 전체 **151 passed**(148+3).
- 충실성 전/후 전체 eval(구 프롬프트 vs 신, 한도 리셋 후 깨끗한 quota):
  - `캐럴-flash-g1` **신 4승·구 0승·무 4**, `캐럴-gemma-g1` **신 4승·구 0승·무 4** → **합 신 8승·구 0승·무 8(완패 없음).** 특히 분석형(Q5~Q8)에서 신 프롬프트를 일관 선호.
  - **표적 환각 제거 확인:** `gemma-g1` Q6의 '난로' 오귀속이 신답에서 사라지고 본문 충실 답변으로 바뀜. (마커 휴리스틱은 '물탱크'·'난로'의 정당한 맥락 언급도 잡아 노이즈 있음 — 실답 확인이 신뢰지표.)
- ⚠️ 측정 중 확인: **flash-lite 실제 한도 = 500/일**(API 429가 `limit:500, model:gemini-3.1-flash-lite, FreeTier`로 명시). flash 대량 eval은 이 벽에 걸리므로 하루 예산 배분 필요. gemma는 별도 1500.
