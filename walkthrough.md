...
### 검증 (일부 — flash 한도 500 소진으로 중단)
- `pytest` 전체 **151 passed**(148+3).
- 충실성 전/후 실측(구 프롬프트 vs 신): `캐럴-flash-g1`에서 **신 3승·구 2승·무 3(리그레션 없음)**. 표적 환각은 확률적이라 이번 baseline엔 미재현 → 강한 실증은 미완.
- ⚠️ **flash-lite 실제 한도 = 500/일(API 429가 명시).** `캐럴-gemma-g1` 구간에서 한도 소진으로 측정 중단. **전체 전/후 eval은 태평양 자정 리셋 후 재실행 예정.**
