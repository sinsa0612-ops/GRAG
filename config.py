...
class Settings(BaseSettings):
    ...
    # --- 글로벌 검색 재설계(신뢰성) 설정 ---
    # MAP 투입 기준: 최고 유사도의 이 비율 이상인 리포트를 전부 넣는다(고정 개수 K가 아니라 상대 기준).
    # 고정 K가 광역 질문("전체 내용 종합해줘")을 죽이던 것을 고친다 — 그런 질문은 어떤 리포트와도
    # 특별히 가깝지 않아, 오히려 '독서 기록'처럼 일반적인 리포트가 상위를 차지하고 정작 내용이 있는
    # 리포트가 6~13위로 밀려 K=5에 못 든다(실측). 상대 기준은 질문에 따라 폭이 저절로 변한다 —
    # 좁은 질문은 1위가 압도적이라 소수만, 광역 질문은 점수가 몰려 있어 다수가 들어온다.
    # 0.55는 실측으로 고른 값(리포트 14·질문 12 표본에서 광역 recall 12/12 달성한 최대값). 같은 표본에서
    # 좁은 질문 평균 투입은 3.4로 옛 고정 K=5보다 오히려 적다. 표본이 작으니 실코퍼스에서 재확인 대상.
    global_search_relative_ratio: float = 0.55
    # 투입 하한 — 랭킹이 부정확해도 최소 이만큼은 MAP에 넣어 "재료 0" 실패를 원천 차단(리포트가 이보다 적으면 전량).
    global_search_min_reports: int = 3
    # 투입 상한 — 로컬 모델은 리포트 1개당 MAP 1콜(수십 초)이라, 리포트가 아주 많은 컬렉션에서
    # 광역 질문 하나가 무한정 길어지는 것을 막는 안전판. 잘릴 때는 조용히 넘어가지 않고 경고를 남긴다.
    global_search_max_reports: int = 20
