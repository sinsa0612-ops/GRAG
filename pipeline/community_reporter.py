...
# 아무 것도 하지 않는다. 매번 컬렉션의 리포트를 통째로 새로 계산해 교체한다(M3는 증분 재요약 최적화
# 없이 단순하게 전체 재생성 — 멤버십이 안 바뀐 커뮤니티만 건너뛰는 최적화는 M5 범위).
# 반환값 = 실제로 저장한 리포트 수.
def generate_reports(collection: str, model: str | None = None) -> int:
    ...
    reports: dict[str, dict] = {}  # community_id -> {title, summary, rating, community_id, level}
